...
INSTALL_DIR="${HOME}/.obsidian"   # default installation directory
...
if [[ $ARCHITECTURE = "NO CHECK" ]]; then
  ...
  showHelp "ARM systems are not supported by this Obsidian AppImage installer."
  exit 1
elif [[ $ARCHITECTURE =~ .*i386.*|.*i686.* ]]; then
  showHelp "32-bit systems are not supported by Obsidian."
  exit 1
fi
...
if [[ $LIBFUSE == "" ]]; then
  print "${COLOR_RED}Error: Can't get FUSE on system, please install fuse-libs through dnf${COLOR_RESET}"
  exit 1
fi
...
if [[ "$INCLUDE_PRE_RELEASE" == true ]]; then
  RELEASE_VERSION=$($DL - "https://api.github.com/repos/obsidianmd/obsidian-releases/releases" | grep -Po '"tag_name": ?"v\K.*?(?=")' | sort -rV | head -1)
else
  RELEASE_VERSION=$($DL - "https://api.github.com/repos/obsidianmd/obsidian-releases/releases/latest" | grep -Po '"tag_name": ?"v\K.*?(?=")')
fi
...
TEMP_DIR=$(mktemp -d)
$DL "${TEMP_DIR}/Obsidian.AppImage" "https://github.com/obsidianmd/obsidian-releases/releases/download/v${RELEASE_VERSION}/Obsidian-${RELEASE_VERSION}.AppImage"
$DL "${TEMP_DIR}/obsidian.svg" "https://obsidian.md/images/obsidian-logo-gradient.svg"

#-----------------------------------------------------
print 'Installing Obsidian...'
# Delete previous version (in future versions obsidian.desktop might not exist)
rm -f "${INSTALL_DIR}"/*.AppImage ~/.local/share/applications/obsidian.desktop "${INSTALL_DIR}/VERSION"

# Creates the folder where the binary will be stored
mkdir -p "${INSTALL_DIR}/"

# Download the latest version
mv "${TEMP_DIR}/Obsidian.AppImage" "${INSTALL_DIR}/Obsidian.AppImage"
...
mv "${TEMP_DIR}/obsidian.svg" ~/.local/share/icons/hicolor/scalable/apps/obsidian.svg
print "${COLOR_GREEN}OK${COLOR_RESET}"
...
if [[ $DESKTOP =~ .*gnome.*|.*kde.*|.*xfce.*|.*mate.*|.*lxqt.*|.*unity.*|.*x-cinnamon.*|.*deepin.*|.*pantheon.*|.*lxde.*|.*i3.*|.*sway.* ]] || [[ `command -v update-desktop-database` ]]; then
  ...
  rm -f "$DESKTOP_FILE_LOCATION/appimagekit-obsidian.desktop"

  # On some systems this directory doesn't exist by default
  mkdir -p "$DESKTOP_FILE_LOCATION"

  # No spaces or tabs should be used for indentation with Bash heredocs
  cat >> "$DESKTOP_FILE_LOCATION/appimagekit-obsidian.desktop" <<-EOF
[Desktop Entry]
Encoding=UTF-8
Name=Obsidian
Comment=Obsidian
Exec=env APPIMAGELAUNCHER_DISABLE=TRUE "${INSTALL_DIR}/Obsidian.AppImage" ${SANDBOXPARAM} %u
Icon=obsidian
